...
class MRV():
    ...
    def select_variable(self, grid):
        ...
        for i in range(w):
            for j in range(w):
                dom_l = len(grid.get_cells()[i][j])
                if dom_l < min_s and dom_l > 1:
                    min_s = dom_l
                    res = (i, j)
        return res
...
class AC3:
    ...
    def consistency(self, grid, Q):
        ...
        while Q:
            ...
            for elem in (self.remove_domain_row, self.remove_domain_column, self.remove_domain_unit):
                var_ls, boolv = elem(grid, i, j)
                if (boolv == True):
                    return False 
                Q.update(var_ls) #updating the queue with consistent data
...
class Backtracking:
    ...
    def search(self, grid, var_selector):
        ...
        curr_v = var_selector.select_variable(grid)
        if curr_v == (None, None):
            return None  #invalid puzzle/not solvable

        i, j = curr_v
        original_domain = grid.get_cells()[i][j]

        A = AC3() #getting instance of AC3 to run consistency()

        i, j = curr_v
        dom = grid.get_cells()[i][j]
        for elem in dom:
            copy_g = grid.copy()
            copy_g.get_cells()[i][j] = elem

            if A.consistency(copy_g, {(i, j)}):
